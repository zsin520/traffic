...
def load_data(data_dir):
    ...
    images=[]
    labels=[]
    for foldername, subfolders, files in os.walk(data_dir):
        for file in files:
            loc=os.path.join(foldername,file)
            image=cv2.imread(loc,cv2.IMREAD_UNCHANGED)
            if loc.endswith('ppm'):
                newImage=cv2.resize(image, [IMG_WIDTH,IMG_HEIGHT])
                images.append(newImage)
                end=foldername[-2:]
                if end.isdigit():
                    labels.append(int(foldername[-2:]))
                else:
                    labels.append(int(foldername[-1]))
...
def get_model():
    ...
    model=tf.keras.models.Sequential([
        ...
        #add a third hidden layer without dropout
        tf.keras.layers.Dense(128, activation='relu'),
